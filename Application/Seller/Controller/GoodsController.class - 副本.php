...
class GoodsController extends BaseController {
            ...
            {                
                ...
                if(!$Goods->create(NULL,$type))// 根据表单提交的POST数据创建数据对象                 
                {
                   ...
                   // C('TOKEN_ON',true);                                                            
                    ...
                    $Goods->goods_type = $type_id = M('goods_category')->where("id = $cat_id3")->getField('type_id'); // 找到这个分类对应的type_id
                    $Goods->store_id = SELLER_ID; // 店家id
                    if ($type == 2)
                    {
                    ...
                    $Goods->afterSave($goods_id,SELLER_ID);
                    
                    
                    $GoodsLogic->saveGoodsAttr($goods_id,$type_id ,SELLER_ID); // 处理商品 属性
    ...
    public function ajaxGetSpecInput(){     
         ...
         $str = $GoodsLogic->getSpecInput($goods_id ,$_POST['spec_arr'],SELLER_ID);
         exit($str);   
    }
