...
class PluginController extends BaseController {
    ...
    public function shipping_close_or_open()
    {
        ...
        if ($is_close == 1) {
            //启动
            $default_c = M('shipping_area')->where("shipping_code = '{$row['code']}' and store_id = " . STORE_ID . " and is_default = 1")->count();
            //判断有没有记录,没有就插入
            if ($default_c == 0) {
        ...
        //不管有没有记录，都启动
        M('shipping_area')->where("shipping_code = '{$row['code']}' and store_id = " . STORE_ID)->data(array('is_close' => $is_close))->save();
...
//        $row = $model->where($condition)->save($update);
    ...
    public function shipping_list(){
        ...
        $sql = "SELECT a.is_default,a.shipping_area_name,a.shipping_area_id AS shipping_area_id,".
            ...
            "FROM __PREFIX__shipping_area a WHERE shipping_code = '{$row['code']}' and store_id = ".STORE_ID;
        //2016-01-11 获取插件信息
        ...
        if(IS_POST){
            ...
            if(I('get.edit') == 1){
                ...
                if($row){
                    ...
                    M('area_region')->addAll($add2)&&exit($this->success("添加成功",U('Plugin/shipping_list',array('type'=>'shipping','code'=>$add['shipping_code']))));
                }
