...
class GoodsController extends BaseController {
    ...
    public function goodsList(){
    	...
    	$filter_goods_id = M('goods')->where("is_on_sale=1 and cat_id{$goodsCate['level']} = $id")->cache(true)->getField("goods_id",true);
    	...
    	if($count > 0)
    	{
    		$goods_list = M('goods')->field('goods_id,cat_id3,goods_sn,goods_name,shop_price,comment_count')->where("goods_id in (".  implode(',', $filter_goods_id).")")->order("$sort $sort_asc")->limit($page->firstRow.','.$page->listRows)->select();
    ...
    */
    ...
    public function guessYouLike(){
        $p = I('p',1);
       $favourite_goods = M('goods')->where("is_recommend=1 and is_on_sale=1")->order('goods_id DESC')->page($p,10)->getField('goods_id,goods_sn,goods_name,shop_price,comment_count');//首页/购物车/我的 推荐商品
       $goods = array();
